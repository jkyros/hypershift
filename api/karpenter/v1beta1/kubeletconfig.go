...
// ToKubeletConfigManifest converts the KubeletConfig spec into a KubeletConfig
// machineconfiguration.openshift.io/v1 YAML manifest string suitable for injection
// into a ConfigMap "config" key. Returns empty string if KubeletConfig is nil.
// The name argument is used as the KubeletConfig CR name.
func (kc *KubeletConfig) ToKubeletConfigManifest(name string) (string, error) {
	if kc == nil {
		return "", nil
	}
