...
type fakeEC2Client struct {
	...
	describeVpcEndpointServicesOut *ec2.DescribeVpcEndpointServicesOutput
...
func (f *fakeEC2Client) DescribeVpcEndpointServicesWithContext(ctx aws.Context, in *ec2.DescribeVpcEndpointServicesInput, o ...request.Option) (*ec2.DescribeVpcEndpointServicesOutput, error) {
	return f.describeVpcEndpointServicesOut, nil
}

func (f *fakeEC2Client) DescribeSubnetsWithContext(ctx aws.Context, in *ec2.DescribeSubnetsInput, o ...request.Option) (*ec2.DescribeSubnetsOutput, error) {
	return f.describeSubnetsOut, nil
}
...
func TestListSubnetIDs(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			subnets, err := listSubnetIDs(t.Context(), fakeClient, nil, "", tc.clusterName, tc.namespace)
			g.Expect(err).NotTo(HaveOccurred())
			g.Expect(subnets).To(Equal(tc.expectedSubnets))
		})
	}
...
func TestFilterSubnetsByEndpointServiceAZs(t *testing.T) {
	testCases := []struct {
		name                string
		endpointServiceName string
		candidateSubnetIDs  []string
		ec2Client           *fakeEC2Client
		expectedSubnets     []string
		expectError         bool
	}{
		{
			name:                "When all subnets are in supported AZs it should return all subnets",
			endpointServiceName: "com.amazonaws.vpce.us-east-1.vpce-svc-abc",
			candidateSubnetIDs:  []string{"subnet-aaa", "subnet-bbb"},
			ec2Client: &fakeEC2Client{
				describeVpcEndpointServicesOut: &ec2.DescribeVpcEndpointServicesOutput{
					ServiceDetails: []*ec2.ServiceDetail{
						{AvailabilityZones: []*string{aws.String("us-east-1c"), aws.String("us-east-1d")}},
					},
				},
				describeSubnetsOut: &ec2.DescribeSubnetsOutput{
					Subnets: []*ec2.Subnet{
						{SubnetId: aws.String("subnet-aaa"), AvailabilityZone: aws.String("us-east-1c")},
						{SubnetId: aws.String("subnet-bbb"), AvailabilityZone: aws.String("us-east-1d")},
					},
				},
			},
		...
		{
			name:                "When a subnet is in an unsupported AZ it should be filtered out",
			endpointServiceName: "com.amazonaws.vpce.us-east-1.vpce-svc-abc",
			candidateSubnetIDs:  []string{"subnet-aaa", "subnet-bad"},
			ec2Client: &fakeEC2Client{
				describeVpcEndpointServicesOut: &ec2.DescribeVpcEndpointServicesOutput{
					ServiceDetails: []*ec2.ServiceDetail{
						{AvailabilityZones: []*string{aws.String("us-east-1c"), aws.String("us-east-1d")}},
				...
				describeSubnetsOut: &ec2.DescribeSubnetsOutput{
					Subnets: []*ec2.Subnet{
						{SubnetId: aws.String("subnet-aaa"), AvailabilityZone: aws.String("us-east-1c")},
						{SubnetId: aws.String("subnet-bad"), AvailabilityZone: aws.String("us-east-1a")},
					},
				},
			},
		...
		{
			name:                "When all subnets are in unsupported AZs it should return empty list",
			endpointServiceName: "com.amazonaws.vpce.us-east-1.vpce-svc-abc",
			candidateSubnetIDs:  []string{"subnet-bad1", "subnet-bad2"},
			ec2Client: &fakeEC2Client{
				describeVpcEndpointServicesOut: &ec2.DescribeVpcEndpointServicesOutput{
					ServiceDetails: []*ec2.ServiceDetail{
						{AvailabilityZones: []*string{aws.String("us-east-1c"), aws.String("us-east-1d")}},
				...
				describeSubnetsOut: &ec2.DescribeSubnetsOutput{
					Subnets: []*ec2.Subnet{
						{SubnetId: aws.String("subnet-bad1"), AvailabilityZone: aws.String("us-east-1a")},
						{SubnetId: aws.String("subnet-bad2"), AvailabilityZone: aws.String("us-east-1b")},
					},
				},
			},
			expectedSubnets: nil,
		...
		{
			name:                "When endpoint service has no service details it should return an error",
			endpointServiceName: "com.amazonaws.vpce.us-east-1.vpce-svc-missing",
			candidateSubnetIDs:  []string{"subnet-aaa"},
			ec2Client: &fakeEC2Client{
				describeVpcEndpointServicesOut: &ec2.DescribeVpcEndpointServicesOutput{
					ServiceDetails: []*ec2.ServiceDetail{},
				},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := filterSubnetsByEndpointServiceAZs(ctx, tc.ec2Client, tc.endpointServiceName, tc.candidateSubnetIDs)
			if tc.expectError {
				g.Expect(err).To(HaveOccurred())
			} else {
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(result).To(Equal(tc.expectedSubnets))
			}
		})
	}
}

func TestListSubnetIDsWithAZFilter(t *testing.T) {
	g := NewWithT(t)

	// NodePool subnet is in a supported AZ (use1-az1).
	// Karpenter subnet-karpenter-good is in supported AZ (use1-az2).
	// Karpenter subnet-karpenter-bad is in unsupported AZ (use1-az4) and should be dropped.
	objects := []client.Object{
		&hyperv1.NodePool{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "nodepool-1",
		...
		&corev1.ConfigMap{
			...
			Data: map[string]string{
				"subnetIDs": `["subnet-karpenter-good","subnet-karpenter-bad"]`,
			},
		},
	}
	...
	ec2Client := &fakeEC2Client{
		...
		describeSubnetsOut: &ec2.DescribeSubnetsOutput{
			Subnets: []*ec2.Subnet{
				{SubnetId: aws.String("subnet-karpenter-good"), AvailabilityZone: aws.String("us-east-1c")},
				{SubnetId: aws.String("subnet-karpenter-bad"), AvailabilityZone: aws.String("us-east-1a")},
			},
		},
	}

	ctx := log.IntoContext(t.Context(), testr.New(t))
	subnets, err := listSubnetIDs(ctx, fakeClient, ec2Client, "com.amazonaws.vpce.us-east-1.vpce-svc-abc", "my-cluster", "clusters")
	g.Expect(err).NotTo(HaveOccurred())
	// subnet-nodepool is included (NodePool subnet, not filtered).
	// subnet-karpenter-good is included (supported AZ).
	// subnet-karpenter-bad is excluded (unsupported AZ use1-az4).
	g.Expect(subnets).To(ConsistOf("subnet-nodepool", "subnet-karpenter-good"))
}
