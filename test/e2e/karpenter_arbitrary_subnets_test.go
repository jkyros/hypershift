...
// NLB backing the private endpoint, so the controller's filterSubnetsByEndpointServiceAZs
// pass-through logic (which correctly filters out incompatible subnets in production) does
// not drop it.
//
// Validation steps:
...
func TestKarpenterArbitrarySubnets(t *testing.T) {
	...
	e2eutil.NewHypershiftTest(t, ctx, func(t *testing.T, g Gomega, mgtClient crclient.Client, hostedCluster *hyperv1.HostedCluster) {
		...
		t.Run("ConfigMap subnet aggregation", func(t *testing.T) {
			...
			// This guarantees the controller's filterSubnetsByEndpointServiceAZs will
			// not drop the subnet, and ModifyVpcEndpoint won't get an AZ mismatch error.
